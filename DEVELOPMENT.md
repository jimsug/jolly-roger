...
Jolly Roger is written in TypeScript using the Meteor web framework.
To run Jolly Roger locally, first [install Meteor](https://www.meteor.com/install).

> If you are new to this codebase, start with [`docs/onboarding/`](docs/onboarding/), which
> explains Meteor, the architecture and the puzzlehunt domain from scratch. This file is a
> terser reference for people who already know their way around.
...
> [!IMPORTANT]
> Jolly Roger runs on Meteor 3 (see `.meteor/release`), which requires
> [Node.js](https://nodejs.org/) v20 or newer. The production Docker image
> installs Node 22 and the deploy workflow uses Node 20. If you're running as
> a regular process, consider using a version manager like
> [nvm](https://github.com/nvm-sh/nvm), [Volta](https://volta.sh/),
> [asdf](https://asdf-vm.com/) or (my favorite) [mise](https://mise.jdx.dev/)
> to pin the version per project.
