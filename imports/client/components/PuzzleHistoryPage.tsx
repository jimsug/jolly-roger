...
const PuzzleHistoryPage = () => {
  useBreadcrumb({ title: "My Puzzles", path: `/mypuzzles` });
  const userId = this.userId!;
  const puzzleHistoryLoading = useTypedSubscribe(puzzleHistoryForUser, {
    userId,
  });
  const loading = puzzleHistoryLoading();

  useTracker (() => {
    const callActivities = CallActivities.find({user: userId}).fetch();
    const documentActivities = DocumentActivities.find({user: userId}).fetch();
    const chatMessages = ChatMessages.find({sender: userId}).fetch();
  }, [
    loading,
  ]);
...
export default PuzzleHistoryPage;
