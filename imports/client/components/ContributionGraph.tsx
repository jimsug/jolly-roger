...
const ContributionGraph = ({
  ...
  const getColorVar = (totalCount: number, maxTotalCount: number) => {
    if (totalCount === 0) return colorVars[0] ?? "#ebedf0";
    if (maxTotalCount === 0) return colorVars[1] ?? "#9be9a8";
    const intensityRatio = totalCount / maxTotalCount;
    if (intensityRatio <= 0) return colorVars[0] ?? "#ebedf0";
    if (intensityRatio <= 0.25) return colorVars[1] ?? "#9be9a8";
    if (intensityRatio <= 0.5) return colorVars[2] ?? "#40c463";
    if (intensityRatio <= 0.75) return colorVars[3] ?? "#30a14e";
    return colorVars[4] ?? "#216e39";
  };
