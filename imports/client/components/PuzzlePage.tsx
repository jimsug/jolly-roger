  ...
  ({
    ...
    useEffect(() => {
      if (parentId) {
        ...
        while (currentParentId && depth < 3) {
          const parentMessage = ChatMessages.findOne(currentParentId) as
            | FilteredChatMessageType
            | undefined;
          if (parentMessage) {
            parents.push(parentMessage);
            nextParent = parentMessage.parentId;
            currentParentId = parentMessage.parentId ?? undefined;
          } else {
        ...
        setNextParentId(nextParent ?? undefined);
    ...
    const reactionCounts = useMemo(() => {
      ...
      reactions.forEach((reaction) => {
        const child = reaction.content.children[0];
        const emoji = child && nodeIsText(child) ? child.text : "?";
        ...
        if (userId && !userEmojiMap.get(emoji)!.has(userId)) {
    ...
    const reactionUsers = useMemo(() => {
      ...
      reactions.forEach((reaction) => {
        ...
        users.set(
          emoji,
          (users.get(emoji) || new Set()).add(
            (reaction.sender ? displayNames.get(reaction.sender) : undefined) ??
              "???",
          ),
        );
      });
    ...
    const handleReactionClick = (emoji: string) => {
      const existingReaction = userReactions.find((reaction) => {
        const child = reaction.content.children[0];
        return !!child && nodeIsText(child) && child.text === emoji;
      });
        ...
        ? createPortal(
            ...
            >
              <EmojiPicker
                ...
                theme={theme.basicMode as EmojiTheme}
              />
            </EmojiPickerContainer>,
            document.body,
          )
        ...
        <ReactionContainer>
          {Array.from(reactionCounts.entries()).map(([emoji, count]) => {
            const userHasReacted = userReactions.some((reaction) => {
              const child = reaction.content.children[0];
              return (
                child !== undefined && nodeIsText(child) && child.text === emoji
              );
            });
  ...
  >(
    (
      ...
      useImperativeHandle(forwardedRef, () => ({
        ...
        clearInput: () => fancyEditorRef.current?.clearInput(),
        insertImage: (url, id, status) =>
          fancyEditorRef.current?.insertImage(url, id, status),
        replaceImage: (url, id, status) =>
          fancyEditorRef.current?.replaceImage(url, id, status),
      }));
  ...
  ({
    ...
    const tabDefinitions = useMemo(() => {
      const messagesById = chatMessages.reduce((mp, c) => {
        mp.set(c._id, c);
        return mp;
      }, new Map<string, FilteredChatMessageType>());
    ...
    useEffect(() => {
      if (tabDefinitions.length > 0 && !activeTabKey) {
        setActiveTabKey(tabDefinitions[0]?.key ?? null);
...
const ChatSection = React.forwardRef(
  (
    ...
    const inputRef = useRef<FancyEditorHandle>(null);
...
const PuzzlePageMetadata = ({
  ...
  const checkTagLayout = useCallback(() => {
    if (actionRowRef.current) {
      ...
      const currentActionPos = actionButtonsRef.current?.clientHeight ?? 0;
...
const PuzzleGuessModal = React.forwardRef(
  (
    ...
    const onDirectionInputChange = useCallback((value: number) => {
      setHaveSetDirection(true);
      setDirectionInput(value);
    }, []);

    const onConfidenceInputChange = useCallback((value: number) => {
...
const PuzzlePage = React.memo(() => {
  ...
  const canAddPuzzle = useTracker(
    () => Puzzles.findOneAllowingDeleted(puzzleId),
    [puzzleId],
  );
  if (!canAddPuzzle) {
    // This is just to satisfy lint if it's unused, or maybe it should be used?
    // Actually canAddPuzzle seems unused in the code I viewed.
  ...
  const roles = useMemo(
    () =>
      selfUser && huntId ? listAllRolesForHunt(selfUser, { _id: huntId }) : [],
    [selfUser, huntId],
  );
  ...
  const onResize = useCallback((_e?: Event) => {
  ...
  useEffect(() => {
    ...
    setSidebarWidth(
      Math.min(
        sidebarWidth,
        (puzzlePageDivRef.current?.clientWidth ?? MinimumDocumentWidth + 100) -
